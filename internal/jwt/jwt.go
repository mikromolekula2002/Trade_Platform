...
func CreateToken(userLogin string, jwtKey string) (string, error) {
	if jwtKey == "" {
...
func DeleteToken(userLogin string, jwtKey string) (string, error) {
	if jwtKey == "" {
		return "", fmt.Errorf("jwt.go: CreateToken: пустой jwt ключ")
	}
...
func VerifyToken(tokenString, jwtKey string) error {
	claims := &Claims{}
...
func ExtractToken(tokenString, jwtKey string) (*Claims, error) {
	token, err := jwt.ParseWithClaims(tokenString, &Claims{}, func(token *jwt.Token) (interface{}, error) {
		return []byte(jwtKey), nil
	})
	if err != nil {
